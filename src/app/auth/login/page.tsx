    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #0f172a 100%)', padding:'16px', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ width:'100%', maxWidth:'420px', position:'relative', zIndex:1 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'32px' }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.45)', fontSize:'14px', marginTop:'6px', margin:'6px 0 0' }}>by Fortitude Sistym</p>
        </div>
          ...
          <form onSubmit={handleLogin} style={{ display:'flex', flexDirection:'column', gap:'18px' }}>
            ...
            <div>
              ...
              <input
                ...
                required autoComplete="email" placeholder="seu@email.com" disabled={carregando}
                style={{ width:'100%', border:'1.5px solid #e5e7eb', borderRadius:'10px', padding:'11px 14px', fontSize:'14px', outline:'none', boxSizing:'border-box', background:carregando?'#f9fafb':'white', transition:'border-color .15s' }}
                onFocus={e=>{(e.target as HTMLInputElement).style.borderColor='#6366f1'}}
                onBlur={e=>{(e.target as HTMLInputElement).style.borderColor='#e5e7eb'}}
              />
            </div>

            {/* Senha com botao mostrar/ocultar */}
            <div>
              ...
              <div style={{ position:'relative' }}>
                <input
                  ...
                  required autoComplete="current-password" placeholder="Sua senha" disabled={carregando}
                  style={{ width:'100%', border:'1.5px solid #e5e7eb', borderRadius:'10px', padding:'11px 46px 11px 14px', fontSize:'14px', outline:'none', boxSizing:'border-box', background:carregando?'#f9fafb':'white', transition:'border-color .15s' }}
                  onFocus={e=>{(e.target as HTMLInputElement).style.borderColor='#6366f1'}}
                  onBlur={e=>{(e.target as HTMLInputElement).style.borderColor='#e5e7eb'}}
                />
                <button
                  ...
                  style={{ position:'absolute', right:'12px', top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'#9ca3af', display:'flex', alignItems:'center', justifyContent:'center', padding:'4px' }}
            ...
            <button
              ...
              style={{ width:'100%', background:carregando?'#a5b4fc':'linear-gradient(135deg, #6366f1, #4f46e5)', color:'white', border:'none', borderRadius:'12px', padding:'13px', fontSize:'15px', fontWeight:'700', cursor:carregando?'not-allowed':'pointer', marginTop:'4px', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px', letterSpacing:'-0.2px', boxShadow:carregando?'none':'0 4px 14px rgba(99,102,241,0.4)', transition:'all .2s' }}
            >
              {carregando && (
