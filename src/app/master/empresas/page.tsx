...
function formVazio() { return { nome:'', cnpj:'', email:'', telefone:'', endereco:'', plano:'profissional', status:'ativo', vencimento:'', bloqueada: false, motivo_bloqueio:'', whatsapp_habilitado: false, financeiro_habilitado: false, auto_agenda_habilitado: false, finalizar_sem_pagamento: false, valor_mensal:'', dia_vencimento:'' } }
              ...
              <div key={e.id} style={{ background:'white', borderRadius:'12px', padding:'14px 16px', border:`1px solid ${e.is_trial ? '#e0f2fe' : '#f0f0f8'}`, display:'flex', alignItems:'center', gap:'12px', flexWrap:'wrap' }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'4px', flexWrap:'wrap' }}>
                    ...
                    {e.finalizar_sem_pagamento && <span style={{ fontSize:'10px', fontWeight:'700', color:'#059669', background:'#ecfdf5', borderRadius:'4px', padding:'1px 6px' }}>FP</span>}
        ...
        <div onClick={fecharModal} style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.55)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'16px', backdropFilter:'blur(4px)' }}>
          <div onClick={function(ev) { ev.stopPropagation() }} style={{ background:'white', width:'100%', maxWidth:'500px', borderRadius:'18px', maxHeight:'90vh', overflowY:'auto', padding:'22px 20px', boxShadow:'0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 14px', background:'#ecfeff', borderRadius:'10px', border:'1px solid #a5f3fc' }}>
                <div>
                  <p style={{ fontSize:'13px', fontWeight:'600', color:'#0e7490' }}>Habilitar AutoAgenda</p>
                  <p style={{ fontSize:'11px', color:'#9ca3af', marginTop:'1px' }}>O sistema agendará automaticamente clientes toda semana conforme o dia/horário cadastrado</p>
                </div>
                <div onClick={function() { setForm(function(p) { return { ...p, auto_agenda_habilitado: !p.auto_agenda_habilitado } }) }} style={{ width:'44px', height:'24px', borderRadius:'99px', cursor:'pointer', background:form.auto_agenda_habilitado?'#0891b2':'#e5e7eb', position:'relative', flexShrink:0, transition:'background 0.2s' }}>
                  <div style={{ position:'absolute', top:'2px', width:'20px', height:'20px', borderRadius:'50%', background:'white', left:form.auto_agenda_habilitado?'22px':'2px', boxShadow:'0 1px 4px rgba(0,0,0,0.2)', transition:'left 0.2s' }}/>
                </div>
              </div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 14px', background:'#f0fdf4', borderRadius:'10px', border:'1px solid #bbf7d0' }}>
                <div>
                  <p style={{ fontSize:'13px', fontWeight:'600', color:'#065f46' }}>Finalizar sem informar pagamento</p>
                  <p style={{ fontSize:'11px', color:'#9ca3af', marginTop:'1px' }}>Ao finalizar um atendimento, não será exigida a forma de pagamento — finaliza direto</p>
                </div>
                <div onClick={function() { setForm(function(p) { return { ...p, finalizar_sem_pagamento: !p.finalizar_sem_pagamento } }) }} style={{ width:'44px', height:'24px', borderRadius:'99px', cursor:'pointer', background:form.finalizar_sem_pagamento?'#059669':'#e5e7eb', position:'relative', flexShrink:0, transition:'background 0.2s' }}>
                  <div style={{ position:'absolute', top:'2px', width:'20px', height:'20px', borderRadius:'50%', background:'white', left:form.finalizar_sem_pagamento?'22px':'2px', boxShadow:'0 1px 4px rgba(0,0,0,0.2)', transition:'left 0.2s' }}/>
                </div>
              </div>
